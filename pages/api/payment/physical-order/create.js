...
export default async function handler(req, res) {
  ...
  try {
    ...
    const pageCount = book.content?.pages?.length || book.page_count
    const tier = getPhysicalTier(pageCount)
    if (!tier) {
      return res.status(400).json({ error: '이 페이지 수(24~50)는 실물 인쇄 가격 구간에 해당하지 않습니다.' })
    }
    ...
    const productId = process.env[`POLAR_PHYSICAL_BOOK_${tier.id.toUpperCase()}_PRODUCT_ID`]
    if (!productId) {
      console.error(`Missing Polar product id for physical tier ${tier.id}`)
