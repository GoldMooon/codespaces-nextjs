...
export async function generateBookPDF(book) {
  ...
  console.log('[PDF-DEBUG] cwd:', process.cwd())
  console.log('[PDF-DEBUG] fontkit typeof:', typeof fontkit, 'has create:', typeof fontkit?.create)
  const regularPath = path.join(process.cwd(), 'public/fonts/Pretendard-Regular.ttf')
  const boldPath = path.join(process.cwd(), 'public/fonts/Pretendard-Bold.ttf')
  console.log('[PDF-DEBUG] regularPath exists:', fs.existsSync(regularPath), regularPath)
  console.log('[PDF-DEBUG] boldPath exists:', fs.existsSync(boldPath), boldPath)
  pdfDoc.registerFontkit(fontkit)
  const regularFontBytes = fs.readFileSync(regularPath)
  const boldFontBytes = fs.readFileSync(boldPath)
  console.log('[PDF-DEBUG] regularFontBytes length:', regularFontBytes.length)
  console.log('[PDF-DEBUG] boldFontBytes length:', boldFontBytes.length)
  const timesRoman = await pdfDoc.embedFont(boldFontBytes, { subset: true })
  const helvetica = await pdfDoc.embedFont(regularFontBytes, { subset: true })
  console.log('[PDF-DEBUG] timesRoman constructor:', timesRoman.constructor.name)
  console.log('[PDF-DEBUG] helvetica constructor:', helvetica.constructor.name)
