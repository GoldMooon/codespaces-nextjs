...
export const TTS_VOICE = process.env.OPENAI_TTS_VOICE || 'shimmer'
...
export async function retrieveImageResult(client, responseId) {
  ...
  if (response.status === 'failed' || response.status === 'incomplete' || response.status === 'cancelled') {
    return { status: 'failed', base64: null }
  }
...
export async function generateSpeech(client, text) {
  const response = await client.audio.speech.create({
    ...
    // 유치원 선생님이 그림책을 읽어주는 듯한 따뜻하고 또렷한 톤을 지정.
    instructions: '따뜻하고 다정한 목소리로, 유치원 선생님이 아이에게 그림책을 읽어주듯 또박또박 천천히 읽어주세요.',
  })
